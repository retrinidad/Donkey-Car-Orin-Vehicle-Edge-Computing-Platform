...
class SystemMonitor:
    ...
    def run(self, angle, throttle):
        ...
        # Initialize autopilot timer on first run
        if self.autopilot_start is None:
            self.autopilot_start = current
            logger.info("Autopilot started")
            
        if current - self.last_measure >= self.sample_interval:
            # Measure system metrics
            cpu_percent = psutil.cpu_percent()
            memory_percent = psutil.Process().memory_percent()
            latency = current - self.last_measure
            elapsed = current - self.autopilot_start
            
            # Store measurements
            self.cpu_percentages.append(cpu_percent)
            self.memory_percentages.append(memory_percent)
            self.latencies.append(latency)
            self.last_measure = current
            
            # Calculate averages
            avg_cpu = sum(self.cpu_percentages) / len(self.cpu_percentages)
            avg_mem = sum(self.memory_percentages) / len(self.memory_percentages)
            avg_latency = sum(self.latencies) / len(self.latencies)
            
            logger.info(f"System Stats - "
                       f"Runtime: {elapsed:.1f}s, "
                       f"CPU: {cpu_percent:.1f}% (avg: {avg_cpu:.1f}%), "
                       f"Memory: {memory_percent:.1f}% (avg: {avg_mem:.1f}%), "
                       f"Latency: {latency*1000:.2f}ms (avg: {avg_latency*1000:.2f}ms)")
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if model_path:
        ...
        # Add after model inference section but before stop sign detector
        class PrintValues:
            def run(self, angle, throttle):
                print(f"Steering: {angle:.2f}, Throttle: {throttle:.2f}")
                return angle, throttle

        # Add to vehicle pipeline after model inference
        V.add(PrintValues(), 
            inputs=['pilot/angle', 'pilot/throttle'],
            outputs=['pilot/angle', 'pilot/throttle'],
            run_condition='run_pilot')
